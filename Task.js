...
Task.find = function(name) {
    var tasks = _.where(Flixir.tasks, { name: name });

    return tasks[Flixir.config.activeTasks[name]];
};
...
Task.prototype.register = function() {
    ...
    Flixir.config.activeTasks = Flixir.config.activeTasks || {};
    Flixir.config.activeTasks[this.name] = 0;
...
module.exports = function(flixir) {
